...
import { Container, CircularProgress, Box, Stack, Typography, IconButton } from '@mui/material';
...
export default function TransactionsPage() {
    ...
    const { byStoreBoxId, isLoading: loading, error } = useAppSelector((state: any) => state.transactions);

    const currentNetwork = networks.find((n: any) => n.id === selectedNetwork?.id);
    ...
    // Efecto para cargar datos cada vez que se entra o cambia el storeId
    useEffect(() => {
        if (storeId) {
            dispatch(fetchTransactions({ storeId, walletId: appliedWalletFilter, page: 1, limit: pageSize }));
        }
    }, [storeId, appliedWalletFilter, dispatch]);
            ...
            <Container maxWidth="xl" sx={{ position: 'relative', zIndex: 1 }}>
                ...
                <Box sx={{ mb: 6, display: 'flex', alignItems: 'center', gap: 2 }}>
                    ...
                    <PageHeader 
                        ...
                        subtitle={"Red: " + (selectedNetwork?.identification?.name || 'N/A') + " | Store: " + (storeId || 'N/A')}
                    />
                </Box>
